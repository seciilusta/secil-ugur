...
import { couple, engagementCTAs, engagementEvent, engagementHeroInvite, pageMeta } from "@/data/content";
...
export function EngagementHero() {
  ...
  return (
    <section className="relative min-h-[85vh] overflow-hidden lg:min-h-[92vh]">
      ...
      <BotanicalCorner position="top-left" color={theme.decorative} accentColor={theme.accent} className="absolute left-0 top-0" />
      <BotanicalCorner position="top-right" color={theme.decorative} accentColor={theme.accent} className="absolute right-0 top-0" />
      <BotanicalCorner position="bottom-left" color={theme.decorative} accentColor={theme.accent} className="absolute bottom-0 left-0" />
      <BotanicalCorner position="bottom-right" color={theme.decorative} accentColor={theme.accent} className="absolute bottom-0 right-0" />
      ...
      <div className="relative mx-auto grid max-w-6xl items-center gap-10 px-6 py-14 lg:grid-cols-[1fr_1.1fr] lg:gap-16 lg:py-16">
        <div className="order-2 flex flex-col items-center text-center lg:order-1 lg:items-start lg:text-left">
          ...
          >
            ...
            <span className="italic" style={{ color: theme.accent }}>&</span>{" "}
            {couple.firstNames.groom}
          </h1>
          <DecorativeDivider color={theme.text} accentColor={theme.gold} variant="floral" className="mb-5 lg:!mx-0" />
          <p
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            {engagementCTAs.map((cta) => (
              <ThemedButton key={cta.label} href={cta.href} variant="engagement" style={cta.style}>
                {cta.label}
              </ThemedButton>
            ))}
          </div>
        ...
        >
          ...
          <DecorativeDivider color={theme.text} accentColor={theme.accent} variant="floral" className="my-8" />
          <div className="grid grid-cols-2 gap-6 text-center">
            <div>
              <p className={`${sans.className} mb-1 text-[9px] uppercase tracking-[0.3em]`} style={{ color: theme.accent }}>
                Tarih
              </p>
              <p className={`${serif.className} text-lg`}>{engagementEvent.dateDisplay}</p>
            </div>
            <div>
              <p className={`${sans.className} mb-1 text-[9px] uppercase tracking-[0.3em]`} style={{ color: theme.accent }}>
                Saat
              </p>
              <p className={`${serif.className} text-lg`}>{engagementEvent.time}</p>
            </div>
          </div>
        </InvitationFrame>
      </div>
